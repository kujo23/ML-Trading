...
def get_bollinger_bands(rm,rstd):
    ...
    upper_band = rm + rstd*2.1
    lower_band = rm - rstd*1.9
...
def test_run():
    ...
    for i in range(len(upper_band.index)):
        if df['IBM'][i-1] > upper_band.loc[upper_band.index[i-1]] and df['IBM'][i] < upper_band.loc[upper_band.index[i]] and not entryfound:
            ...
            entryfound = True
            signals.append([str(upper_band.index[i]).split()[0],'BUY',])
        # Changing Long Exit Strategy from that of traditional Bollinger Band Long Exit Strategy
        elif df['IBM'][i-1] < upper_band.loc[upper_band.index[i-1]] and df['IBM'][i] > upper_band.loc[upper_band.index[i]] and entryfound:
            long_exits.append(upper_band.index[i])
            entryfound = False
            signals.append([str(upper_band.index[i]).split()[0],'SELL'])

    ordersfile = open('orders1.csv','w')
    ...
    ax = df['IBM'].plot(title='My Strategy Bands',label='IBM')
    rm.plot(label='SMA',ax = ax)
    upper_band.plot(label = 'Upper my_strategy Bands',ax = ax, color='c')
    lower_band.plot(label = 'Lower my_strategy Bands',ax = ax, color='c')
